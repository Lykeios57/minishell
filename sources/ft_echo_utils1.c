...
/*   Updated: 2023/02/28 18:25:33 by lreille          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_interpret(t_parsing_node *parse, t_env *envi, int status)
{
	...
	while (now)
	{
		...
		{
			tmp = now->next->str;
			now->next->str = interpret(tmp, envi, status);
			ft_free(tmp);
		}
		if (now->next)
		{
...
char	*interpret(char *str, t_env *envi, int status)
{
	...
	new_str = ft_malloc(sizeof(char) * (get_len_ev(str, envi, status) + 1));
	tmp = new_str;
	i = 0;
	while (str[i])
	{
		...
		{
			i++;
			i += key_to_value(&str[i], envi, &value, status);
...
int	key_to_value(char *str, t_env *envi, char **value, int status)
{
	...
	while (str[key] && (ft_isalpha(str[key]) || ft_isdigit(str[key])))
		key++;
	*value = get_value_by_key(envi, str, key);
	if (key == 0 && str[0] == '?')
	{
